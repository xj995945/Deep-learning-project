...
vector<STrack> BYTETracker::update(const vector<Object>& objects)
{
	...
	u_track.clear();
	u_detection.clear();
	linear_assignment(dists, dist_size, dist_size_size, 0.2, matches, u_track, u_detection);
	...
	u_detection.clear();
	linear_assignment(dists, dist_size, dist_size_size, 0.2, matches, u_unconfirmed, u_detection);
